...
const MultipleSelect = (foods,foodIds) => {
					...
					<div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
						<div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
							<h3 className="text-xl font-semibold text-gray-900 dark:text-white">
								Pilihan Makanan yang Sering Dimakan
							</h3>
							<button
								type="button"
						...
						<div className="p-4 md:p-5 space-y-4 overflow-auto">
                            <ul className="max-h-64">
                                {foods.foods.map((row) => (
                                    <li className="mb-2 " key={`li_`+row.id}>
                                        <label className="inline-block w-full">
                                            <input type="checkbox" className="mr-3" name="food_ids[]" value={row.id}/>
                                            {row.name}
                                        </label>
                                    </li>
                                ))}
                            </ul>
                            <input type="hidden" required name="food_ids[]"/>
						</div>
